...
for (const collectionType of ["ERC1155", "ERC721"]) {
    ...
    describe(`Testing Marketplace Smart Contract for ${collectionType} Collection`, () => {
        ...
        function isErc1155(contract: IERC1155 | IERC721): contract is IERC1155 {
        ...
        async function tSafeTransferFrom(
            ...
            const mockCollection = await getMockCollection();
            if (isErc1155(mockCollection)) {
                await mockCollection.safeTransferFrom(from, to, nftId, 1, "0x");
            } else {
                await mockCollection.transferFrom(from, to, nftId);
            }
        }
        ...
        class NftSaleHelper {
            ...
            async setupAndMakeOffer(
                ...
                await tSafeTransferFrom(signer.address, seller.address, nftId);
        ...
        async function approveAndListingByASeller(
            ...
            try {
                let marketplace =
                    getMarketplaceFromSignerIndex(sellerSignerIndex);
        ...
        describe("List functions's tests.", () => {
            ...
            beforeEach(async () => {
                ...
                collectionAddress = mockCollectionDeployment.address;
            });
            ...
            it("If an user owner of a NFT try to list that nft before it grants to Marketplace rigths over its token, then an exception should be thrown.", async () => {
                ...
                await expect(
                    ...
                    `${collectionType}: caller is not token owner or approved`
                );
            });
            ...
            it("should revert if price is 0", async () => {
                ...
                await expect(
                    ...
                    "Marketplace: Price must be greater than 0"
                );
            });
        });
        ...
        describe("MakeOffer function's tests. ", () => {
            ...
            it("After a buyer make an offer, he hasn't the NFT's ownership yet.", async () => {
                ...
                const actualDifference =
                    balanceOfMarketplaceAfterMakeOffer?.sub(
        ...
        describe("TakeCounteroffer function tests", () => {
            ...
            it("should increase NFT balance of buyer by 1", async () => {
                ...
                const balanceBefore = await tBalanceOf(buyer.address, nftId);

                await marketplace.takeCounteroffer(1, necessaryAmountToSend);

                const balanceAfter = await tBalanceOf(buyer.address, nftId);
            ...
            it("should decrease NFT balance of marketplace by 1", async () => {
                ...
                const balanceBefore = await tBalanceOf(
                    marketplace.contract.address,
                    nftId
                );
                ...
                const balanceAfter = await tBalanceOf(
                    marketplace.contract.address,
                    nftId
                );
        ...
        describe("ChangePriceOf function tests", () => {
            ...
            it("should change price of NFT", async () => {
                await marketplace.changePriceOf(
                    collectionAddress,
                    nftId,
                    newPrice
                );
        ...
        describe("TakeOffer function's tests", () => {
            ...
            async function presetRequirementsForThreeExpirationDays(
                ...
                await approveAndListingByTheOwner(
                    collectionAddress,
                    nftId,
                    price
                );
        ...
        describe("CancelOffer function's tests", () => {
            ...
            beforeEach(async () => {
                ...
                imNotTheBuyer = await getAnotherSigner(
            ...
            it("When an offer is cancelled, the money that collateralize it, it's go out from the Marketplace Contract.", async () => {
                ...
                const actualMarketplaceBalance =
                    await ethers.provider.getBalance(
                        marketplaceDeployment.address
                    );
        ...
        describe("SetFeeRatioFromPercentage function tests", () => {
            ...
            it("should change value of feeRatio", async () => {
                const NEW_PERCENTAGE = 30;
                await expect(
                    marketplace.setFeeRatioFromPercentage(NEW_PERCENTAGE)
                ).to.be.not.reverted;
