...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const cap: number = 9;
  const feeNumerator: number = 1000;
  const name: string = "WarInMars";
  const symbol: string = "Weapons";
  await deploy("FusyERC721CollectionWithRoyaltySupport", {
    from: creator,
    autoMine: true,
    log: true,
    args: [uri, cap, name, symbol, feeNumerator],
  });
};
export default func;
func.tags = ["FusyERC721CollectionWithRoyaltySupport"];
